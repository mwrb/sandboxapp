  ...
  class TuiParser < ApplicationService
    ...
    def call
      ...
      offers = data["offers"].map do |d|
        ...
        # puts ""
        # offers.group_by { |o| o[:country] }.map { |k,v|
        #   puts "#{k} - #{v.map{|c| c[:city]}.uniq}"
        # }
        #
        next if d["boardType"] == "Bez wyżywienia"
        {
      ...
      offers.sort_by { |x| [  x[:country], x[:standard] ] }
    end
  end
end
